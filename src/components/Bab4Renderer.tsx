...
export const Bab4Renderer = ({ theme, username, userClass, title, onComplete, searchQuery }: any) => {
  ...
  const [step, setStep] = useState(() => {
    if (isGuru) return 0;
    if (userKey) {
      const saved = localStorage.getItem(`ipa_bab4_step_${userKey}`);
  ...
  const [scores, setScores] = useState<number[]>(() => {
    ...
    if (userKey) {
      const saved = localStorage.getItem(`ipa_bab4_scores_${userKey}`);
  ...
  const [highestStep, setHighestStep] = useState(() => {
    if (isGuru) return 0;
    if (userKey) {
      const saved = localStorage.getItem(`ipa_bab4_highestStep_${userKey}`);
      return saved ? parseInt(saved, 10) : 0;
    }
    return 0;
  });
  ...
  useEffect(() => {
    if (!isGuru && userKey) {
      const savedStep = localStorage.getItem(`ipa_bab4_step_${userKey}`);
      setStep(savedStep ? parseInt(savedStep, 10) : 0);

      const savedScores = localStorage.getItem(`ipa_bab4_scores_${userKey}`);
      setScores(savedScores ? JSON.parse(savedScores) : [0, 0, 0, 0]);

      const savedHighest = localStorage.getItem(`ipa_bab4_highestStep_${userKey}`);
  ...
  useEffect(() => {
    if (!isGuru && userKey) {
      localStorage.setItem(`ipa_bab4_step_${userKey}`, step.toString());
    }
  }, [step, userKey, isGuru]);

  useEffect(() => {
    if (!isGuru && userKey) {
      localStorage.setItem(`ipa_bab4_scores_${userKey}`, JSON.stringify(scores));
    }
  }, [scores, userKey, isGuru]);

  useEffect(() => {
    if (!isGuru && userKey) {
      localStorage.setItem(`ipa_bab4_highestStep_${userKey}`, highestStep.toString());
    }
  ...
  const renderStep = () => {
    switch(step) {
      ...
      case 1: return <Kuis1Bab4 theme={theme} isGuru={isGuru} onSuccess={(s: number) => handleKuisSuccess(0, s)} onRetry={() => {}} />;
      case 2: return <Kuis2Bab4 theme={theme} isGuru={isGuru} onSuccess={(s: number) => handleKuisSuccess(1, s)} onRetry={() => {}} />;
      case 3: return <Kuis3Bab4 theme={theme} isGuru={isGuru} onSuccess={(s: number) => handleKuisSuccess(2, s)} onRetry={() => {}} />;
      case 4: return <Kuis4Bab4 theme={theme} isGuru={isGuru} onSuccess={(s: number) => handleKuisSuccess(3, s)} onRetry={() => {}} />;
  ...
  return (
    <div className="w-full flex flex-col items-center justify-start pt-2 pb-6 px-2 sm:px-4">
      {/* Tabs Menu */}
      <div className="w-full max-w-4xl mb-3 sm:mb-6 bg-blue-900/80 p-1 sm:p-1.5 rounded-xl sm:rounded-2xl flex items-stretch justify-between gap-1 shadow-lg backdrop-blur-md border border-white/10 overflow-hidden">
        {TABS.map((tab, idx) => {
