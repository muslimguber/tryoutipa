...
export const Kuis3Bab4: React.FC<KuisProps> = ({ theme, onSuccess, onRetry }) => {
  const [questions] = useState(Array.from({ length: 20 }).map((_, idx) => ({
    id: idx + 1,
    text: `Contoh Soal ${idx + 1} Kuis 3 Bab 4 (Akan diganti nanti)`,
    options: [
      { id: 'a', text: 'Pilihan A' },
      { id: 'b', text: 'Pilihan B' },
      { id: 'c', text: 'Pilihan C' },
  ...
  if (showResult) {
    ...
    return (
      <div className="bg-white/90 backdrop-blur-md p-8 rounded-3xl text-center shadow-xl border border-white/20">
        <h2 className="text-3xl font-black text-slate-800 mb-2">Hasil Kuis 3 Bab 4</h2>
